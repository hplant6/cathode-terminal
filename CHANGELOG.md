...
### Fixed
- **A property added from the Box/Lasso panel's picker is now scrolled into view.** "Add a CSS property" is a sticky footer pinned to the bottom of the drawer, with an opaque background so properties scroll underneath it. New fields append to the end of the User Added section — which is exactly the spot the footer covers — so the property you just added was hidden behind the control you added it from, and the only way to see it was to scroll down and find it yourself. The drawer now scrolls the new field clear of the footer and flashes it once, so the result of the click is where the click was. Focus no longer fights the scroll either: the browser's own focus scrolling parks the field flush against the bottom edge of the scrollport, back underneath the footer.
