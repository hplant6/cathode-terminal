...
### Added
- **The active project in Your Projects is two cards tall and shows its screenshot.** Every other project card led with a screenshot of the project, and the one you were actually working in was the only card without one — so the project you knew best was the hardest to spot. The active card now carries its screenshot directly under its title bar, above the server table, and spans two rows of the grid so the servers keep the room they had. It holds that height in a single-column window too, where there are no cards beside it to set the row height.
