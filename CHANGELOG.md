...
### Fixed
- **A CSS property you add in the Box/Lasso panel now appears where you added it.** The User Added picker sits under every detected property, and the element list only shows 300px at a time — so on an element with a full property list the new row was rendered somewhere below the fold, and adding one read as nothing having happened. People were falling back on the filter box to find the property they had just added. The new row is now scrolled into view and flashes briefly, and the section is ordered by when you added things rather than by the underlying property list, so the most recent one is always the row directly above the picker. That last part also fixes re-adding a property that was already detected, which used to reappear in its original position part-way up the list.
