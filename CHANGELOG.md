...
### Changed
- **"Send as Intent" is a button between Cancel and Send, not a switch.** As a toggle it was sticky state living on a bar you weren't looking at: you set it once for one message and it stayed on, or you meant it and found it off. It is a second send now, decided at the moment of sending, which is the only moment the choice means anything — same message, same panel, with the intent framing in front of it. It appears in every tool panel's footer and follows Send's enabled state, so it isn't offered where a plain send isn't. The directive it prepends has been rewritten to be shorter and more direct: snap raw values to the nearest existing tokens, variables or utility classes; change things at the component or stylesheet level rather than inline; keep the result visually equivalent, and explain it when doing it properly doesn't look the same.
