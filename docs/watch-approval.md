...
Gamut Toolbox can mirror a risky tool-permission prompt to a phone/watch companion app, so
...
agent wants a risky tool
   ...
   └─▶ relay POST /api/request ─▶ APNs ─▶ phone/watch ┘ the loser is cancelled
                      wrist tap ─▶ relay ─▶ Gamut Toolbox polls ─▶ resolves the same prompt
```
...
- Everything is **best-effort**: if the feature is off, the relay is unreachable, or the
  request times out, Gamut Toolbox falls back to the in-app modal with no change in behavior.
- If you answer in-app, Gamut Toolbox calls `POST /api/cancel/{id}` so a late wrist tap cleanly
...
Or edit the config file directly — a `watch-approval.json` in Gamut Toolbox's user-data directory:

| OS | Path |
|---|---|
| Windows | `%APPDATA%\Gamut Toolbox\watch-approval.json` |
| macOS | `~/Library/Application Support/Gamut Toolbox/watch-approval.json` |
| Linux | `~/.config/Gamut Toolbox/watch-approval.json` |
...
| `pollMs` | `1500` | How often Gamut Toolbox polls the relay for the wrist decision. |
