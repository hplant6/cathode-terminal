...
Gamut Terminal can mirror a risky tool-permission prompt to a phone/watch companion app, so
...
agent wants a risky tool
   ...
   └─▶ relay POST /api/request ─▶ APNs ─▶ phone/watch ┘ the loser is cancelled
                      wrist tap ─▶ relay ─▶ Gamut Terminal polls ─▶ resolves the same prompt
```
...
- Everything is **best-effort**: if the feature is off, the relay is unreachable, or the
  request times out, Gamut Terminal falls back to the in-app modal with no change in behavior.
- If you answer in-app, Gamut Terminal calls `POST /api/cancel/{id}` so a late wrist tap cleanly
...
Or edit the config file directly — a `watch-approval.json` in Gamut Terminal's user-data directory:

| OS | Path |
|---|---|
| Windows | `%APPDATA%\Gamut Terminal\watch-approval.json` |
| macOS | `~/Library/Application Support/Gamut Terminal/watch-approval.json` |
| Linux | `~/.config/Gamut Terminal/watch-approval.json` |
...
| `pollMs` | `1500` | How often Gamut Terminal polls the relay for the wrist decision. |
